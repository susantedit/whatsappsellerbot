...
// Set these in your server's .env, php.ini, or hosting panel.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_SERVER['HTTPS']) === false && getenv('APP_ENV') === 'production') {
        http_response_code(403);
        echo json_encode(["status" => "error", "message" => "HTTPS required"]);
        exit;
    }

    // ── Rate limiting: max 5 OTP requests per IP per 10 minutes ──
    $ip         = $_SERVER['REMOTE_ADDR'] ?? 'unknown';
    $rate_file  = sys_get_temp_dir() . '/otp_rate_' . md5($ip) . '.json';
    ...
    if ($rate_data['count'] >= $max_req) {
        ...
        error_log("[SECURITY] OTP rate limit hit for IP: {$ip} at " . date('Y-m-d H:i:s'));
        exit;
    }
    ...
    // Security: OTP is generated SERVER-SIDE, not trusted from client
    $to  = filter_var($input['email'], FILTER_SANITIZE_EMAIL);
    $otp = strval(random_int(100000, 999999)); // cryptographically secure

    // Store OTP server-side with expiry (10 minutes)
    $otp_file = sys_get_temp_dir() . '/otp_' . md5($to) . '.json';
    ...
    if (!$username || !$password) {
        ...
        echo json_encode(["status" => "error", "message" => "Mail not configured"]);
        error_log("[SECURITY] SMTP credentials missing");
        exit;
    }
    ...
    if ($result === true) {
        // Return only success — never return the OTP in the response
        echo json_encode(["status" => "success"]);
        error_log("[AUTH] OTP sent to {$to} from IP {$ip}");
    } else {
        http_response_code(500);
        echo json_encode(["status" => "error", "message" => "Failed to send email"]);
        error_log("[ERROR] SMTP failure for {$to}: {$result}");
    }
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    ...
    $otp_in   = preg_replace('/[^0-9]/', '', $input['otp']); // digits only
    $otp_file = sys_get_temp_dir() . '/otp_' . md5($email) . '.json';
    ...
    $stored = json_decode(file_get_contents($otp_file), true);
    ...
    if (!password_verify($otp_in, $stored['otp'])) {
        http_response_code(401);
        echo json_encode(["status" => "error", "message" => "Invalid OTP"]);
        error_log("[AUTH] Failed OTP attempt for {$email} from IP " . ($_SERVER['REMOTE_ADDR'] ?? 'unknown'));
        exit;
    }
...
// ── SMTP function (SSL verification ENABLED) ──────────────────
function send_smtp_mail($to, $subject, $body, $user, $pass, $host, $port) {
    // SSL verification enabled — do NOT disable in production
